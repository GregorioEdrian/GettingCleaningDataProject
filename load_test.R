load_test <- function(x){
        ...
        ## body_gyro_x_test.tx, body_gyro_y_test.txt, body_gyro_z_test.txt,
     ...
     axialtest <- c("_x_test.txt","_y_test.txt","_z_test.txt") ## axis of the activities. 
     descriproutData <- c("body_acc", "body_gyro", "total_acc") ## activities
     descripData <- c() ## vector for all names of variables what will be create.
     
     ## Are created part of the route to each  of the files and are stored within a vector. 
     ## For example, "UCI HAR Dataset/test/Inertial Signals/_x_test.txt"
